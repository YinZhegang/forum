  ...
  <div class="post">
    <div class="input">
       <el-form ref="form" :model="form" >
         <el-form-item label="帖子标题" label-width="80px">
           <el-input @submit.native.prevent size="mini" placeholder="请输入标题不多于40个字" v-model.trim="form.title"></el-input>
         </el-form-item>
         <el-form-item>
           <el-input  :rows="7" type="textarea" placeholder="请输入内容不超过500字" v-model="form.content"></el-input>
<!--           <p :style="{backgroundImage: 'url(' + emoji + ')',width:'25px',height:'25px',backgroundSize:'100% 100%',margin:'5px'}"></p>-->
         </el-form-item>
       </el-form>
    </div>
    <div class="imgUpload">
      <div class="uploaded">
            <div v-for="item, index in form.files" class="item" >
                <i @click="delImg(index)" class="el-icon-circle-close" style="position: absolute;right: -10px;top: -5px;background-color: #f6f6f6;border-radius: 20px;z-index: 1"></i>
               <img class="uploadImg" :style="{maxWidth:itemWidth+'px',maxHeight:itemWidth+'px',zIndex:-1}" :src='item.filePath' :preview="item.fileOrder"  />
            </div>
        <div class="plust" v-if="form.files.length<9" :style="{width:itemWidth+'px',height:itemWidth+'px'}">
          <input type="file" :disabled="!isUploaded" id="uploadfile" multiple @change="isUploaded&&previewImg($event)" style="position: absolute;width: 100%;height: 100%;left: 0;opacity: 0;z-index: 999" />
       </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit () {
      ...
      if (this.$route.query.sid) {
        ...
        axios.post(this.baseURL + 'suggest/update',
          {
        ...
        axios.post(this.baseURL + 'suggest/add', this.form, {headers: {'Content-Type': 'application/json'}}
        ).then(res => {
          if (res.data.code === 200) {
            Toast('创建成功')
            setTimeout(function () {
  ...
  mounted () {
    this.$store.state.headerZindex = 9999
    var that = this
    this.tabHeader.title = '发帖子'
    this.itemWidth = Math.floor(document.body.clientWidth / 3.4)
