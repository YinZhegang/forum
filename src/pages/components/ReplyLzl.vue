    ...
    <div class="lzl">
      ...
      <div class="cont" v-if="replies.total">
        <p style="white-space: pre-wrap;">
          <span v-for="item in replies.records" :style="{color:item.fromUser.owner?'#26a2ff':'#000000' }" @click="commentLzl(item)">{{item.fromUser.name}} <span>回复 {{item.toUser.name}}：</span> {{item.content}}<br /></span>
         </p>
...
export default {
  ...
  props: ['replies', 'reviewId', 'likeCount', 'like', 'moduleId', 'mouduleOwn', 'forbid', 'reviewOwner'],
  methods: {
    thumbsUp () {
